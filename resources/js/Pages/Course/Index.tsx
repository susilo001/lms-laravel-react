...
import { Course, PageProps, PaginationProps } from "@/types";
import {
  ArrowLeftIcon,
  ArrowRightIcon,
  BookmarkSquareIcon,
...
export default function CoursePage({
  auth,
  courses,
}: PageProps<{ courses: PaginationProps<Course> }>) {
  const prevPageUrl = courses.links.prev;
  const nextPageUrl = courses.links.next;
    ...
    <AuthenticatedLayout>
      ...
      <section className="space-y-10">
              ...
              <CardFooter className="border-t p-4">
                <div className="flex items-center justify-between ">
                  ...
                  <div className="flex items-center gap-2">
                    <Chip value={course.category.name} variant="gradient" />
                    <Tooltip content={`${course.modules?.length} Modules`}>
                      <Chip
        ...
        >
          ...
          {courses.meta.links.map((link, index) => (
            <IconButton
              key={index}
              onClick={() => router.get(link.url ? link.url : "#")}
              className={`${
                link.active ? "bg-blue-gray-100 text-blue-gray-900" : ""
              } ${
